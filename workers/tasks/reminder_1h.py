...
async def run(
    ...
    """Send reminder 1 hour before intake."""
    ...
    for course in courses:
        ...
        try:
            await tg_retry(
                ...
                text=WorkerTemplates.reminder_1h(intake_str),
            )
            await mark_sent(redis, course.id, REMINDER_TYPE)
            logger.info("Reminder 1h → user=%d, course=%d", user.telegram_id, course.id)
        except TelegramForbiddenError:
            logger.info("Girl blocked bot, telegram_id=%d", user.telegram_id)
        except Exception:
            logger.exception("Failed to send 1h reminder to %d", user.telegram_id)
